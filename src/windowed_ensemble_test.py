...
for i in experiment_dicts:
  ...
  #      exprmt_feature_vector.append(i[k])
    if not i.has_key(label):
        continue
    #elif  i[label]>4:
    #    continue
    elif i[label]<label_min:
        continue
    elif i[label]>label_max:
        continue
...
#        clf=GradientBoostingRegressor(n_estimators=100,learning_rate=i)
        ...
        Y_C_predicted=np.array(Y_R_predicted)<label_cutoff
...
for n_estimators in [10,50,100]:
    for depth in [6,8,12]:#,8,12]:
        clf =RandomForestClassifier(n_estimators=n_estimators,max_depth=depth,
                min_samples_split=5, random_state=0)#min per split was 10.
    #    clf = AdaBoostClassifier(n_estimators=n_estimators)
        ...
        for i in range(0,len(keys)*len(ranges)):
            if i%len(ranges) ==0:
                print ""
