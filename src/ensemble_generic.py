...
for l in exons_file:
    ...
    if mean_exon_label(exon_dict)<min_mean_label:
        continue
    for key in exon_dict.keys():
        if type(exon_dict[key]) == dict:
            if len(exon_dict[key]) < len(keys): 
...
for n_estimators in [10,50,150]:
    for depth in [8,12]:
        clf =RandomForestClassifier(n_estimators=n_estimators,max_depth=depth,
                min_samples_split=10, random_state=0)#min per split was 10
