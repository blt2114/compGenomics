...
if len(sys.argv) != 3:
    sys.stderr.write("invalid usage: python merge_ChIP.py  <dir> <files_to_merge.txt>\n")
    sys.exit(2)
...
for l in files[0]:
    ...
    for f in files[1:]:
        ...
        if site_dict["location"] != new_site_dict["location"]:
            ...
            if site_dict.has_key(key):
                for s_key in new_site_dict[key].keys():
                    if not site_dict[key].has_key(s_key):
                        ...
                        sys.stderr.write("key: "+key+"-"+s_key+" defined in"
                                +" multiple files")
            else:
