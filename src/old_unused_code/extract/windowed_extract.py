...
Jeff modified this program to run as a function
"""
...
# Main Method
def main(argv):
    windowed_extract(argv[0], argv[1], argv[2])
...
def windowed_extract(config_fn, sites_json_fn, data_fn):
    ...
    sample_mark = data_fn.split(".")[0].split("-")
    ...
    print current_site["read_dir"]

    #calculate new site information
    if current_site["read_dir"] == 1:
        win_start = int(current_site["five_p_loc"]) - upstream_window
        win_end = int(current_site["five_p_loc"]) + downstream_window
    else:
        win_start = int(current_site["five_p_loc"]) - downstream_window
        win_end = int(current_site["five_p_loc"]) + upstream_window
